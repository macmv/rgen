...
use crate::{Placer, Random, Result, Rng, rng};
...
impl Default for WideCanopyJungle {
  fn default() -> Self {
    Self {
      ...
      roots:       block!(log[variant="jungle",axis="none"]), /* block!(log[variant="jungle"]),/
                                                               * /block![concrete[color="green"
                                                               * ]], */
...
impl WideCanopyJungle {
      ...
      for dx in -1..=1 {
        for dz in -1..=1 {
          ...
          if world.get(pos) == block![air]
            || world.get(pos) == block![water]
            || world.get(pos) == self.leaves
          {
  ...
  fn make_mid_branches(&self, world: &mut PartialWorld, rng: &mut Rng, center: Pos) -> Vec<Pos> {
    ...
    for &(dx, dz) in &directions {
      ...
      let third_pos = if rng.range(0..3) == 0 {
        // Veer left or right
        let (off_dx, off_dz) = if rng.range(0..2) == 0 { (dx, 0) } else { (0, dz) };
    ...
    for rotate in 0..4 {
      ...
      for z in -2i32..=2 {
        for x in 0..=1 {
          ...
          if height > 0 {
            if !(height == 1
              && (world.get(Self::rotate_face(pos + Pos::new(0, -1, 0), base, rotate))
                == block![air]
                || world.get(Self::rotate_face(pos + Pos::new(0, -1, 0), base, rotate))
                  == block![water]))
            {
  ...
  fn special_check_ground(
    ...
    if !touched_ground {
      ...
      loop {
        loop_y += 1;
        if world.get(Self::rotate_face(pos + Pos::new(0, -1 * loop_y, 0), base, rotate))
          == block![air]
          || world.get(Self::rotate_face(pos + Pos::new(0, -1 * loop_y, 0), base, rotate))
            == self.leaves
          || world.get(Self::rotate_face(pos + Pos::new(0, -1 * loop_y, 0), base, rotate))
            == block![water]
        {
  ...
  fn place_root_lines(&self, world: &mut PartialWorld, rng: &mut Rng, base: Pos) {
    ...
    'outer: for &(dx, dz) in directions.iter() {
      ...
      let tail_options: Vec<(i32, i32)> =
        directions.iter().cloned().filter(|&(x, z)| (x, z) != opposite).collect();
