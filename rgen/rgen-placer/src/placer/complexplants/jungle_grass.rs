...
use crate::{Rng, Placer, Random, Result}; //rng::Random
        ...
        for dz in -radius..=radius {
          for dy in -4..4{
            ...
            if !(distannce>radius as f64) {
              if self.place_above.contains(world.get(set_pos)) && world.get(set_pos+Pos::new(0,1,0))==block!(air) && world.get(set_pos+Pos::new(0,2,0))==block!(air) && world.get(set_pos+Pos::new(0,3,0))==block!(air){
                ...
                if chance < flower_percent{
                  ...
                  if grass_chance < 4{
                    ...
                    // Double Tall Grass
                    world.set(set_pos+Pos::new(0,1,0), self.tall_grass.with_prop("half", "lower").with_prop("variant", "double_grass"));
                    world.set(set_pos+Pos::new(0,2,0), self.tall_grass.with_prop("half", "upper").with_prop("variant", "sunflower"));
                  }
                  else if grass_chance < 7 {
                    // Double Tall Fern
                    world.set(set_pos+Pos::new(0,1,0), self.tall_grass.with_prop("half", "lower").with_prop("variant", "double_fern"));
                    world.set(set_pos+Pos::new(0,2,0), self.tall_grass.with_prop("half", "upper").with_prop("variant", "sunflower"));
                  }
